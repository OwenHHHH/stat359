...
# sequence processing function - constructing tensor of shape (32,300) as specified
def tokenize_and_pad(sentences, model, max_len=32):
    embedding_dim = model.vector_size
    num_samples = len(sentences)
    # Initializing tensor(Padding)
    X_tensor = np.zeros((num_samples, max_len, embedding_dim), dtype=np.float32)
    # go through each sentence
    for i, sentence in tqdm(enumerate(sentences), total=num_samples, desc="Encoding sequences"):
        words = sentence.lower().split()
        # Truncate if longer than max_len
        if len(words) > max_len:
            words = words[:max_len]
        # update tensore with word's context vector
        for t, word in enumerate(words):
            try:
                X_tensor[i, t, :] = model[word]
            except KeyError:
                # keep zero vector
                pass
    ...
    return X_tensor
# encoding sentences
X_seq = tokenize_and_pad(data['sentence'], fasttext)

# splitting the data - trainval and test, then splitting trainval to train and val
...
val_loader = DataLoader(SequenceDataset(X_val, y_val), batch_size=BATCH_SIZE, shuffle=False) # keeping val/test non-random
...
class LSTMClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_classes, dropout):
        ...
        # Fully connected layer for converting hidden layer into class prediction
        self.fc = nn.Sequential(
            nn.Linear(hidden_dim, num_classes)
        )
    # forward pass
    def forward(self, x):
        # getting the hidden state from lstm
        out, (h_n, c_n) = self.lstm(x)
        # getting last hidden state
        last_hidden = h_n[-1, :, :]
        # passing through MLP to get final hidden state to class prediction
        logits = self.fc(last_hidden)
        return logits
# Initialize Model
device = get_device()
input_dim = 300
model = LSTMClassifier(input_dim, HIDDEN_DIM, NUM_LAYERS, 3, DROPOUT).to(device)

# setting loss and optimizer including class weighting
...
# training model
train_loss_hist, val_loss_hist = [], [] # initializing values
...
# Separate Accuracy Plot to match given py files
